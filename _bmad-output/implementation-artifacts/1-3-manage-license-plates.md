...
- [ ] Task 1: Define the vehicle-profile contract on top of the existing schema (AC: 1, 2, 4, 5)
  - [ ] Add the missing Pydantic schemas for vehicle create/read/delete flows under `backend/src/app/schemas`.
  - [ ] Decide plate normalization rules for MVP, including trimming, casing, and duplicate comparison behavior.
  - [ ] Keep the API response small and mobile-friendly, returning only fields needed by Story 1.3 and later check-in flows.

- [ ] Task 2: Implement backend vehicle management for the authenticated driver (AC: 1, 2, 3, 4, 5)
  - [ ] Add driver-scoped vehicle endpoints under the existing users/profile domain, reusing `backend/src/app/api/v1/users.py` unless a cleaner vehicle submodule is clearly better.
  - [ ] Resolve the current authenticated `user` to its linked `driver` record before reading or mutating vehicle data.
  - [ ] Support listing current vehicles, creating a vehicle, and deleting a vehicle.
  - [ ] Reject duplicate license plates with the backend exception style already used by auth and users.

- [ ] Task 3: Add a minimal mobile vehicle management flow for the public account shell (AC: 1, 2, 3)
  - [ ] Introduce a simple vehicle management screen reachable from the authenticated public flow.
  - [ ] Show the current list of saved plates and provide a compact add/remove UI.
  - [ ] Preserve the current authenticated session and map prototype while adding this screen.

- [ ] Task 4: Guard business rules needed by upcoming parking flows (AC: 3, 4, 5)
  - [ ] Prevent deleting a vehicle that is still referenced by future protected flows if the current backend rules require it; otherwise document the temporary MVP behavior.
  - [ ] Ensure only driver-capable public accounts can manage vehicle records.
  - [ ] Make error states explicit enough for mobile to surface actionable messages.

- [ ] Task 5: Test backend and mobile vehicle management (AC: 1, 2, 3, 4, 5)
  - [ ] Add backend tests for list/create/delete, duplicate rejection, and driver ownership enforcement.
  - [ ] Add at least one Flutter test covering list rendering or add/remove behavior.
  - [ ] Smoke-test the end-to-end flow on the current Android device/backend setup used for Story 1.2.
...
- Story prepared as the next implementation target after authenticated login is complete on mobile and backend.
- Planning assumes reuse of the existing `Vehicle` SQLAlchemy model and the current authenticated driver identity.
- Scope is intentionally limited to vehicle list/create/delete so later parking stories can build on stable data first.

### File List

- `_bmad-output/implementation-artifacts/1-3-manage-license-plates.md`
